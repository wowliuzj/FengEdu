...
use app\education\models\MessageSearch;
...
class MessageController extends Controller
{
    ...
    public function actionIndex()
    {
        ...
        $type = $session['USER_SESSION']['ftype'];
        if($type == 5){
            $params['sid'] = $sid;
        }else if($type == 3){
            $params['csid'] = $sid;
    ...
    public function actionDeletes($ids)
    {
        ...
        $response = Yii::$app->response;
        $response->format = \yii\web\Response::FORMAT_JSON;
        $sql = "delete from ".Message::tableName()." where id in(".$ids.")";
        ...
        */
        ...
        $response = Yii::$app->response;
        $response->format = \yii\web\Response::FORMAT_JSON;
        $sql = "delete from ".Message::tableName()." where id in(".$strIds.")";
        $res = Yii::$app->db->createCommand($sql)->execute();
        if($res == 0){
            $response->data = \Tool::toResJson(0, "找不到该记录，删除失败");
        }else{
