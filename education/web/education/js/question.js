    ...
    if(responseText.s == 1){
        ...
        var cnt = '<tr>\
                ...
                <td style="text-align:center">\
                    <button type="button" onclick="javascript:edit(\'#it_id#\')">查看</button>\
                    <button type="button" onclick="javascript:view(\'#it_id#\')">预览</button>\
                    <button type="button" onclick="javascript:addQuestion(\'#it_id#\')">添加</button>\
                </td>\
               </tr>';
        data_body.empty();
        for (key in list)
...
function edit(id){
    document.location.href="index.php?r=/education&page=question/answer_index&id="+id;
}
function view(id){
